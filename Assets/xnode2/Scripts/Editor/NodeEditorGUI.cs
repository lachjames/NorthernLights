...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        void DrawControls ()
        {
            using (new GUILayout.AreaScope(new Rect(0, 0, 128, 192)))
            {
                using (new GUILayout.VerticalScope())
                {
                    ...
                    if (curNode == null)
                    {
                        GUILayout.Label("Cur: None");
                    }
                    else
                    {
                        GUILayout.Label("Cur: " + curNode.name);
                    }
                }
            }
        }
        ...
        void StartGraph ()
        {
            curNode = GetStartNode();
        }

        AuroraNode GetStartNode ()
        {
            foreach (Node raw in graph.nodes)
            {
                AuroraNode node = (AuroraNode)raw;
                if (node.GetType().Name == "OnBeginScript")
                {
                    return node;
                }
            }
        ...
        void StepGraph ()
        {
            ...
            curNode = curNode.Execute();
            Debug.Log("Moved to " + curNode);
        }
        ...
        void UpdateActions ()
        {
            foreach (XNode.Node node in graph.nodes)
            {
                ...
                if (node.GetType().Name == "ActionNode")
                {
                    node.GetType().GetMethod("UpdateAction").Invoke(node, new object[] { });
                } else if (node.GetType().Name == "CustomNode")
                {
                    node.GetType().GetMethod("UpdateAction").Invoke(node, new object[] { });
                }
            }
